...
public class MessageController {
    ...
    private static final String ROOM_ID = "1408";
    ...
    public void getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        ...
        if (roomId.equals(ROOM_ID)) {
            logger.warn("Attempt to send message from read-only room {}", roomId);
            return;
        }

        saveMessage(roomId, message).subscribe(msgId -> logger.info("message send id:{}", msgId));
        Message msgToSend = new Message(HtmlUtils.htmlEscape(message.messageStr()));
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId), msgToSend);
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, ROOM_ID), msgToSend);
    }
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
        ...
        if (ROOM_ID.equals(String.valueOf(roomId))) {
